...
dfA2_typed <- dfA2 |>
  mutate(
    # parse_number handles stray spaces and grouping marks; adjust decimal/grouping if needed
    DataValue   = parse_number(as.character(DataValue)),
    depthUpp    = parse_number(as.character(depthUpp)),
    depthLow    = parse_number(as.character(depthLow)),
    LogDistance = parse_number(as.character(LogDistance)),
    year        = suppressWarnings(as.integer(parse_number(as.character(year)))),
    rec         = parse_number(as.character(rec))
  )



# Sum nasc over different depths (in 1st version the depth is not accounted for)
tot_nasc_per_log<-dfA2_typed |> group_by(year, rec, LogDistance) |> 
...
tot_nasc_per_log_plus_one<-pA1 |> select(-area_NM2, -A_NM2) |> 
  full_join(plus_one) |>
  mutate(sum_nasc=ifelse(sum_nasc==0, 1, sum_nasc))

#print(x=tot_nasc_per_log_plus_one |> filter(year==2020, rec==1), n=100)
#print(x=tot_nasc_per_log_plus_one, n=10000)
